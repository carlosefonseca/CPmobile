...
$depart = (!$debug)? normalize($_GET["departure"])	: "alhandra";
$arrival= (!$debug)? normalize($_GET["arrival"]) 	: "oriente";
$day 	= (!$debug)? normalize($_GET["day"])		: date("o-m-d");
$hour 	= (!$debug)? normalize($_GET["hour"])		: date("G");
...
$f;
if (!$debug || !file_exists("data.html")) {
if ($debug) { echo "\nfresh data\n"; }
